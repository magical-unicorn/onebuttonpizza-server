...
//file_put_contents('./step1.txt', serialize($_REQUEST));
	$params = '?name='.urlencode($_GET['name']).'&amp;pizza='.urlencode($_GET['pizza']).'&amp;phone='.urlencode($_GET['phone']).'&amp;address='.urlencode($_GET['address']);
?><Response>
<Gather timeout="10" finishOnKey="*" action="http://obp.sous-anneau.org/results.php<?php echo $params; ?>">
<Say language="fr" voice="woman">Bonjour. Ici la COGIP. <?php echo $_GET['name']; ?>  aimerait commander une pizza <?php echo $_GET['pizza']; ?>.
